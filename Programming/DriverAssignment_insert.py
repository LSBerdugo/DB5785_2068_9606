...
bus_ids = list(range(1, 140))  # BusIDs

start_date = datetime(2023, 1, 1)
end_date = datetime(2025, 1, 1)

# Open SQL file to write inserts
with open("DriverAssignment_insert.sql", "w") as file:
    file.write("INSERT INTO DriverAssignment (AssignmentID, BusID, AssignmentDate, EndDate, StaffID) VALUES\n")

    values = []
    for i in range(400):
        assignment_id = i + 1
        bus_id = random.choice(bus_ids)
        assignment_date = start_date + timedelta(days=random.randint(0, (end_date - start_date).days))
        end_date_val = assignment_date + timedelta(days=random.randint(1, 30)) if random.random() > 0.2 else None
        staff_id = random.choice(driver_staff_ids)

        if end_date_val:
            values.append(f"({assignment_id}, {bus_id}, '{assignment_date.strftime('%Y-%m-%d')}', '{end_date_val.strftime('%Y-%m-%d')}', {staff_id})")
        else:
            values.append(f"({assignment_id}, {bus_id}, '{assignment_date.strftime('%Y-%m-%d')}', NULL, {staff_id})")

    file.write(",\n".join(values) + ";\n")

print("SQL insert statements written to DriverAssignment_insert.sql")