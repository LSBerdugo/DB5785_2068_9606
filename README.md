...
##### הסבר על דרך העבודה והכלים שהשתמשנו בהם ליצירת האפליקציה
...
# הוראות הפעלה של האפליקציה:
1.יש לוודא כי מותקנת גרסה עדכנית של פייתון (3.8 ומעלה):
✅ Python 3.8+ must be installed on the system.

2.יש לוודא התקנה של הספריות הבאות:

psycopg2 – לחיבור למסד הנתונים PostgreSQL

tkinter – ספריית GUI הכלולה כברירת מחדל בפייתון

3.יש להפעיל את מסד הנתונים PostgreSQL ולוודא שהטבלאות הדרושות קיימות:

Staff, Driver, Schedule, Attendance,
Salary, DriverAssignment, DriverLicense


4.יש לפתוח את הקובץ הראשי ולהריץ את הקוד באמצעות שורת הפקודה:



4. יש לפתוח את הקובץ הראשי main.py ולהריץ את הקוד, למשל באמצעות שורת הפקודה:

python main.py

5.לאחר התחברות מוצלחת למסד הנתונים – ייפתח הממשק הגרפי של המערכת:
משם ניתן לגשת לכל חלקי הניהול: עובדים, נהגים, שיבוצים...
   
   ---
# הסבר על דרך העבודה והכלים שהשתמשנו בהם ליצירת האפליקציה:

מבנה האפליקציה:
האפליקציה פותחה בשפת Python תוך שימוש בגישת הפרדה בין רכיבי הקוד (מודולריות). המבנה הכללי כולל:

ממשק משתמש (GUI) – מבוסס tkinter, בעיצוב נעים, נגיש וידידותי למשתמש.

חיבור למסד הנתונים – באמצעות מחלקה ייעודית DatabaseConnection המשתמשת ב־psycopg2 לניהול חיבורים ושאילתות מול PostgreSQL.

מודולים נפרדים לפי תחומי אחריות:

StaffManagement – ניהול נתוני עובדים.

DriverManagement – ניהול נתוני נהגים.

ScheduleManagement – ניהול משמרות.

DriverAssignmentManagement –  ניהול הקצאת נהגים לאוטובוסים.

QueriesFunctionsWindow – הרצת שאילתות ופרוצדורות מוכנות מראש לצורכי ניתוח ובקרה.

הכלים בהם נעשה שימוש:
Python 3 – שפת הפיתוח הראשית.

Tkinter – ספרייה מובנית ליצירת ממשקי משתמש גרפיים.

PostgreSQL – מערכת ניהול מסדי נתונים רלציונית .

psycopg2 – ממשק חיבור בין פייתון ל־PostgreSQL.


מהלך הפיתוח:
הפיתוח התבצע לאחר בניית בסיס נתונים , והתמקד ביצירת ממשק גרפי לניהול המידע בעזרת שפת Python וספריית Tkinter.
המטרה הייתה לאפשר למשתמש גישה נוחה לפעולות עיקריות כמו ניהול עובדים, נהגים, שיבוצים, משמרות וצפייה בדוחות.

לשם כך חולקה העבודה למספר רכיבים עצמאיים, כשכל רכיב מטפל בנושא אחר (למשל: DriverManagement, ScheduleManagement).
לכל אחד מהם נבנה ממשק משתמש עם אפשרות להוספה, עדכון, מחיקה וחיפוש.

החיבור למסד הנתונים בוצע באמצעות psycopg2, תוך הקפדה על שימוש בפרוצדורות ושאילתות מתקדמות שהוגדרו מראש.

לבסוף, נבנה תפריט ראשי (MainApplication) שמרכז את כל החלונות השונים של המערכת, בעיצוב נוח, אחיד ונגיש.
